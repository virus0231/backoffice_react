...
try {
    ...
    if ($metric === 'chart') {
        ...
        // Build td join conditions for filters
        $tdJoinConditions = "t.id = td.TID";

        if (!empty($appealIdsArray)) {
            $sanitizedAppealIds = array_map('intval', $appealIdsArray);
            $tdJoinConditions .= " AND td.appeal_id IN (" . implode(',', $sanitizedAppealIds) . ")";
        }

        if (!empty($fundIdsArray)) {
            $sanitizedFundIds = array_map('intval', $fundIdsArray);
            $tdJoinConditions .= " AND td.fundlist_id IN (" . implode(',', $sanitizedFundIds) . ")";
        }

        // Table data query - fetch raw data for frontend calculation
        // Use LEFT JOIN with filters in join condition to preserve zero payment methods
        $sql = "
        SELECT
            pm.paymenttype AS payment_method,
            td.amount,
            td.freq,
            t.totalamount
        FROM (
        ...
        LEFT JOIN pw_transactions t ON t.paymenttype = pm.paymenttype
            AND t.status IN ('Completed', 'pending')
            AND t.date >= {$startDateLiteral}
            AND t.date < DATE_ADD({$endDateLiteral}, INTERVAL 1 DAY)
        LEFT JOIN pw_transaction_details td ON {$tdJoinConditions}
        ORDER BY pm.paymenttype, td.amount";
