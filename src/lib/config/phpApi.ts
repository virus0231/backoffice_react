...
export function getPhpApiBase(clientId?: string): string {
  ...
  const clientUrls: Record<string, string | undefined> = {
    mausa: import.meta.env.NEXT_PUBLIC_API_URL_MAUSA,
    amoud: import.meta.env.NEXT_PUBLIC_API_URL_AMOUD,
    afghan: import.meta.env.NEXT_PUBLIC_API_URL_AFGHAN,
    rusard: import.meta.env.NEXT_PUBLIC_API_URL_RUSARD,
  };
  ...
  // Fallback to default
  const fromEnv = normalizeBase(import.meta.env.NEXT_PUBLIC_PHP_API_BASE_URL);
...
export function buildRecurringPlansUrl(
  metric: string,
  searchParams: URLSearchParams
): string {
  const base = getPhpApiBase();
...
export function buildRecurringRevenueUrl(
  metric: string,
  searchParams: URLSearchParams
): string {
  const base = getPhpApiBase();
...
export function buildFrequenciesUrl(
  metric: string,
  searchParams: URLSearchParams
): string {
  const base = getPhpApiBase();
...
export function buildPaymentMethodsUrl(
  metric: string,
  searchParams: URLSearchParams
): string {
  const base = getPhpApiBase();
...
export function buildFundsDataUrl(
  metric: string,
  searchParams: URLSearchParams
): string {
  const base = getPhpApiBase();
...
export function buildCountriesDataUrl(
  metric: string,
  searchParams: URLSearchParams
): string {
  const base = getPhpApiBase();
