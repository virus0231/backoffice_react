...
function generateSynthetic(start: Date, end: Date, granularity: 'daily' | 'weekly') {
  ...
  for (let i = 0; i < totalDays; i += step) {
    ...
    const base = Math.round(400 + 120 * Math.cos(i / 3));
    out.push({ date: d.toISOString().slice(0, 10), amount: base, count: Math.round(base / 20) });
  }
  return out;
}
...
export async function GET(request: Request) {
  ...
  const scaleForFilters = () => {
    let s = 1;
    if (appealId) s *= 0.98;
    if (fundId) s *= 0.97;
    if (frequency === 'one-time') s *= 0.7; // first-installments lower when filtering one-time
    return s;
  };
  const params = {
    startDate,
  ...
  try {
    const [agg, trendData, cmpAgg, cmpTrend] = await Promise.all([
      aggregateRevenue(params, 'first-installments'),
      revenueTrend(params, 'first-installments', granularity),
      compareStartDate && compareEndDate ? aggregateRevenue({ ...params, startDate: compareStartDate, endDate: compareEndDate }, 'first-installments') : Promise.resolve(null),
      compareStartDate && compareEndDate ? revenueTrend({ ...params, startDate: compareStartDate, endDate: compareEndDate }, 'first-installments', granularity) : Promise.resolve(null)
    ]);
