...
function generateSynthetic(start: Date, end: Date, granularity: 'daily' | 'weekly') {
  ...
  for (let i = 0; i < totalDays; i += step) {
    ...
    const base = Math.round(600 + 150 * Math.sin(i / 4 + 0.5));
    out.push({ date: d.toISOString().slice(0, 10), amount: base, count: Math.round(base / 30) });
  }
  return out;
}
...
export async function GET(request: Request) {
  ...
  try {
    const [agg, trendData, cmpAgg, cmpTrend] = await Promise.all([
      aggregateRevenue(params, 'one-time'),
      revenueTrend(params, 'one-time', granularity),
      compareStartDate && compareEndDate ? aggregateRevenue({ ...params, startDate: compareStartDate, endDate: compareEndDate }, 'one-time') : Promise.resolve(null),
      compareStartDate && compareEndDate ? revenueTrend({ ...params, startDate: compareStartDate, endDate: compareEndDate }, 'one-time', granularity) : Promise.resolve(null)
    ]);
