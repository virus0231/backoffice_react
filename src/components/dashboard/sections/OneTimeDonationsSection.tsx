...
export default function OneTimeDonationsSection({ granularity }: Props) {
  const { data, loading, error } = useAnalytics(`/api/v1/analytics/one-time-donations?granularity=${granularity}`, 'oneTimeDonations');
  ...
  return (
    <ChartWrapper chartId="oneTimeDonations" title="One-time Donations" enableComparison>
      <div className="mb-3 flex items-end justify-between">
        <div>
          <div className="text-sm text-slate-500">Single Gift Revenue</div>
          ...
          <div className="text-xs text-slate-500">Donations: {data?.data?.currentPeriod?.donationCount ?? '—'}</div>
        </div>
        <div>
