jQuery(document).ready(function () {
  ...
  $("#manual_transation_form").submit(function (e) {
    ...
    $("select[name^='freq']").each(function () {
      ...
      if (value === "monthly" || value === "yearly") {
        let planId = $("input[name='" + planIdField + "']")
          .val()
          .trim();
        let subId = $("input[name='" + subIdField + "']")
          .val()
          .trim();
        ...
        if (planId === "" || subId === "") {
          isValid = false;
          if (planId === "")
            $("input[name='" + planIdField + "']").addClass("is-invalid");
          if (subId === "")
            $("input[name='" + subIdField + "']").addClass("is-invalid");
        } else {
          $(
            "input[name='" +
              planIdField +
        ...
        $(
          "input[name='" + planIdField + "'], input[name='" + subIdField + "']"
        ).removeClass("is-invalid");
      }
    });
  ...
  $("#manual_sub_form").on("submit", function (event) {
    ...
    else if (!start_date || isNaN(new Date(start_date).getTime()))
      errors.push("Valid start date is required.");
    else if (appeal_id == 0) errors.push("Appeal ID is required.");
    else if (quantity < 1 || quantity == "")
      errors.push("The quantity should be 1 or greater.");
  ...
  function qurbani_updatePagination(data) {
    ...
    $("#pagination").html(`
        ...
        } class='btn btn-gradient-success waves-effect waves-light' style='background: linear-gradient(14deg, #1eca7b 0%, rgba(30,202,123,0.7));
  ...
  $("#qurbaniDetail").click(function () {
    ...
    $.ajax({
      ...
      data: { qurbaniDetail: true, txtsearch: txtsearch, orderid: orderid },
      success: function (response) {
        if (response) {
          ...
          link.download =
            "AfghanRelief_QurbaniDetailReport_" + dateString + ".csv";
  ...
  function updatePaginationEployee(data) {
    ...
    $("#pagination").html(`
        <button id="prevPage" ${
          currentPageEmployee === 1 ? "disabled" : ""
        } class='btn btn-gradient-success waves-effect waves-light' style='background: linear-gradient(14deg, #1eca7b 0%, rgba(30,202,123,0.7));
    color: #fff;'>Previous</button>
        <span>Page ${currentPageEmployee} of ${totalPages}</span>
        <button id="nextPage" ${
          currentPageEmployee === totalPages ? "disabled" : ""
        } class='btn btn-gradient-success waves-effect waves-light' style='background: linear-gradient(14deg, #1eca7b 0%, rgba(30,202,123,0.7));
    color: #fff;'>Next</button>
    `);
  ...
  $("#employee_export_btn").click(function () {
    ...
    $.ajax({
      ...
      url: "/backend/yoc/getReportData.php",
      data: {
        employee_export_btn: "employee_export_btn",
        startDate: employee_start_date,
        endDate: employee_end_date,
        email: employee_email,
      },
    ...
    $.post(
      ...
      (data, status) => {
        if (data) {
          ...
          link.href = window.URL.createObjectURL(blob);
          link.download =
            "AfghanRelief_DonorDetailReport_" + dateString + ".csv";
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
        } else {
  ...
  $("#btnexport_monthly_report").click(function () {
    ...
    $.ajax({
      ...
      data: { btnexport_monthly_report: true, from: from, to: to },
      success: function (response) {
        if (response) {
          var blob = new Blob([response]);
          var link = document.createElement("a");
  ...
  $("#btnexport_daily_report").click(function () {
    ...
    $.ajax({
      type: "POST",
      url: "/backend/yoc/getReportData.php",
      data: { btnexport_daily: true, from: from, to: to },
      success: function (response) {
        if (response) {
          var blob = new Blob([response]);
          var link = document.createElement("a");
  ...
  jQuery("#generateotp").click(function () {
    ...
    jQuery.ajax({
      ...
      data: { generate_otp: true, email: email },
      dataType: "json",
      success: function (response) {
        if (response.status === "success") {
          let resetUrl =
            "/backend/pages/password/reset_password.php?id=" +
            response.userId +
            "&email=" +
            response.email;
  ...
  jQuery("#verify_otp").click(function () {
    ...
    jQuery.ajax({
      type: "POST",
      url: "/backend/yoc/verification.php",
      data: { verify_otp: true, otp: otp, id: id },
      dataType: "json",
      success: function (response) {
        ...
        if (response.status === "success") {
          ...
          let resetUrl =
            "/backend/pages/password/reset_password.php?reset=" +
            response.userId;
          window.location = resetUrl;
        } else {
          // Show a pop-up or alert when the response status is not "success"
  ...
  jQuery("#new_password").click(function () {
    ...
    jQuery.ajax({
      type: "POST",
      url: "/backend/yoc/verification.php",
      data: { new_password: true, password: password, id: id },
      success: function (response) {
        //  console.log(response);
        ...
        const newData = `
            <tr>
               <td>${i + 1}</td>
                ...
                <td>${items["totalamount"]}</td>
                <td>${items["status"]}</td>
             <td><a href="/backend/pages/donation/DonationDetail.php?reference=${
               items["TID"]
             }" class="btn btn-gradient-success waves-effect waves-light" style='background: linear-gradient(14deg, #1761fd 0%, rgba(23,97,253,0.6)); box-shadow: 3px 3px 20px 0 rgba(23,97,253,0.4);
      ...
      try {
        const newData = `
            <tr>
                ...
                <td>${items["status"]}</td>
          <td><a href="/backend/pages/schedules/ScheduleDetail.php?reference=${
            items["sch_id"]
          }" class="btn btn-gradient-success waves-effect waves-light" style='background: linear-gradient(14deg, #1761fd 0%, rgba(23,97,253,0.6)); box-shadow: 3px 3px 20px 0 rgba(23,97,253,0.4);
    border-radius: 4px;' target="_blank">Detail</a></td>?>
  ...
  function updatePaginationSchedule(data) {
    ...
    $("#pagination").html(`
        <button id="prevPage" ${
          currentPageSchedule === 1 ? "disabled" : ""
        } class='btn btn-gradient-success waves-effect waves-light' style='background: linear-gradient(14deg, #1eca7b 0%, rgba(30,202,123,0.7));
    color: #fff;'>Previous</button>
        <span>Page ${currentPageSchedule} of ${totalPages}</span>
        <button id="nextPage" ${
          currentPageSchedule === totalPages ? "disabled" : ""
        } class='btn btn-gradient-success waves-effect waves-light' style='background: linear-gradient(14deg, #1eca7b 0%, rgba(30,202,123,0.7));
    color: #fff;'>Next</button>
    `);
  ...
  function updatePaginationipn(data) {
    ...
    $("#pagination").html(`
        <button id="prevPage" ${
          currentPageipn === 1 ? "disabled" : ""
        } class='btn btn-gradient-success waves-effect waves-light' style='background: linear-gradient(14deg, #1eca7b 0%, rgba(30,202,123,0.7));
    color: #fff;'>Previous</button>
        <span>Page ${currentPageipn} of ${totalPages}</span>
        <button id="nextPage" ${
          currentPageipn === totalPages ? "disabled" : ""
        } class='btn btn-gradient-success waves-effect waves-light' style='background: linear-gradient(14deg, #1eca7b 0%, rgba(30,202,123,0.7));
    color: #fff;'>Next</button>
    `);
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response) {
          ...
          link.href = window.URL.createObjectURL(blob);
          link.download =
            "afghanRelief_missingcombination_" + dateString + ".csv";
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
        } else {
  ...
  $("#muharramreport").click(function () {
    ...
    $.ajax({
      type: "POST",
      url: "/backend/yoc/getReportData.php",
      data: { muharram: true, orderid: orderid, email: email },
      success: function (response) {
        if (response) {
          var blob = new Blob([response]);
          var link = document.createElement("a");
...
function Add_amount_Record() {
  ...
  html +=
    '<input type="text" class="form-control" name="amount_name_' +
    count +
    '" placeholder="Donation Name" value="">';
  html += "</div>";
  html += '<div class="col-md-1 px-1">';
  html +=
    '<input type="number" class="form-control" name="amount_amount_' +
    count +
    '" placeholder="Amount" value="">';
  html += "</div>";
  html += '<div class="col-md-1 px-1">';
  html +=
    '<input type="number" class="form-control" name="amount_sort_' +
    count +
    '" placeholder="Sort" value="">';
  html += "</div>";
  html += '<div class="col-md-2">';
  html +=
    '<select class="form-control" name="amount_donation_type_' + count + '">';
  ...
  html +=
    '<button type="button" class="btn btn-gradient-primary waves-effect waves-light" onclick="Add_amount_Record()" id="amount_add_' +
    count +
    '">+</button>';
  html +=
    '<button type="button" class="btn btn-gradient-danger waves-effect waves-light" onclick="Rem_amount_Record(this.id)" id="amount_remove_' +
    count +
    '">-</button>';
  html += "</div>";
  html += "</div>";
...
function Add_fund_Record() {
  ...
  html +=
    '<input type="text" class="form-control" name="fund_name_' +
    count +
    '" placeholder="Fund Name" value="">';
  html += "</div>";

  html += '<div class="col-md-2">';
  html +=
    '<input type="number" class="form-control" name="fund_sort_' +
    count +
  ...
  html +=
    '<button type="button" class="btn btn-gradient-primary waves-effect waves-light" onclick="Add_fund_Record()" id="fund_add_' +
    count +
    '">+</button>';
  html +=
    '<button type="button" class="btn btn-gradient-danger waves-effect waves-light" onclick="Rem_fund_Record(this.id)" id="fund_remove_' +
    count +
    '">-</button>';
  html += "</div>";
  html += "</div>";
  ...
  if (isvalidate) {
    ...
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        if (
          response.indexOf("Updated") >= 0 ||
          response.indexOf("Inserted") >= 0
        ) {
          const Toast = Swal.mixin({
            toast: true,
            position: "top-end",
